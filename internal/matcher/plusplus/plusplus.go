...
var pattern = regexp.MustCompile(
	fmt.Sprintf("(^|\\s)([\\p{L}\\w\\+-]*(%s+)[\\p{L}\\w\\+-]*|[\\p{L}\\w\\+-]+)(\\+\\+|\\+-|--|—)", emojiPattern),
)
...
type Matcher struct {
	abstract.Matcher
	cfg  interfaces.PlusplusMatcherConfigStruct
...
func NewMatcher(
	...
	config interfaces.PlusplusMatcherConfigStruct,
	repo interfaces.PlusplusRepoInterface,
) *Matcher {
	return &Matcher{
...
func (m *Matcher) processTokens(chatID int64, tokens []token) (*[]interfaces.TelegramMessageStruct, error) {
	...
	for _, token := range tokens {
		tokenRes, err := m.processToken(chatID, token)
		if err != nil {
			return nil, err
		}
...
func (m *Matcher) processToken(chatID int64, token token) (*interfaces.TelegramMessageStruct, error) {
	...
	reply := reply(token, value)

	return &reply, nil
}

func reply(token token, value int) interfaces.TelegramMessageStruct {
	var mode string
	switch {
	...
	return telegram.NewMarkdownMessage(
		fmt.Sprintf(
			...
			telegram.EscapeMarkdown(token.name),
