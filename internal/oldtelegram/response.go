...
func SendMessage(requestMessage RequestMessage, responseMessage Message) error {
	oldlogger.Log.Infof("send to %d: %s", requestMessage.Chat.ID, responseMessage.Text)
	...
	// Construct API URL
	url := fmt.Sprintf(oldconfig.Cfg.Telegram.BaseUrl, oldconfig.Cfg.Telegram.ApiKey) + oldconfig.Cfg.Telegram.EndpointSendMessage
