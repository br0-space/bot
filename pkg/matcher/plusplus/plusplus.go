...
var pattern = regexp.MustCompile(fmt.Sprintf(`(^|\s)([\p{L}\p{M}\p{S}\p{N}\+-]*(%s+)[\p{L}\p{M}\p{S}\p{N}\+-]*|[\p{L}\p{M}\p{S}\p{N}\+-]+)([+]{2,}|[-]{2,}|\+-|—)`, emojiPattern))
...
func GetTokenIncrements(matches []string) ([]string, map[string]int, error) {
	...
	for _, match := range matches {
		...
		if _, exists := increments[name]; !exists {
			...
			increments[name] = increments[name] + increment
		}
	}
